        ...
        if (FD_ISSET(master_socket, &readfds)){
            ...
            {   
                ...
                {   
                    ...
                    memset( ac[i].addr,0,16*sizeof(ac[i].addr[0]) );
                    memset( ac[i].name,0,13*sizeof(ac[i].name[0]) ); 

                    sprintf(ac[i].addr,"%s", inet_ntoa(address.sin_addr));
                    sprintf(ac[i].name,"anonymous");
                    ac[i].port = ntohs (address.sin_port);
            ...
            for (int i = 0;i < max_clients;i++){
                if (client_socket[i]){
                    ...
                    if (ac[i].port == ntohs(address.sin_port) && !strcmp(inet_ntoa(address.sin_addr), ac[i].addr) ){
                    }else{
            ...
            if (FD_ISSET( sd , &readfds)){   
                ...
                if ((valread = read( sd , buffer, 1025)) == 0){   
                    ...
                    for (int j = 0;j < max_clients;j++){
                        if (client_socket[j]){
                            memset(message,0,1025*sizeof(message[0]));
                            if (ac[j].port == ntohs(address.sin_port) && !strcmp(inet_ntoa(address.sin_addr), ac[j].addr) ){
                    ...
                    if (!strcmp(str,"who\n")){
                        ...
                        for (int z = 0;z < max_clients;z++){
                            if (client_socket[z]){
                                memset(message,0,sizeof(message[0])*1025);
                                sprintf(message,"[Server] %s %s:%d",ac[z].name,ac[z].addr,ac[z].port);
                                if ( z == i ){
                        ...
                        for (int z = 0;z < max_clients;z++){
                            if (client_socket[z]){
                                ...
                                if (!ac[i].name[0])
                                    ...
                                    sprintf(message,"[Server] %s",ac[i].name);
